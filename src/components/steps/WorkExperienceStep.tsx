...
interface WorkExperienceStepProps {
  data: WorkExperience;
  updateData: (data: Partial<WorkExperience>) => void;
  onNext: () => void;
  onPrevious: () => void;
}
...
export const WorkExperienceStep: React.FC<WorkExperienceStepProps> = ({
  ...
  const isValid = data.currentlyWorking !== undefined && 
    data.clinicalAreas.length > 0 && data.responsibilities;
  ...
  const toggleClinicalArea = (area: string) => {
    if (data.clinicalAreas.includes(area)) {
      updateData({ 
        clinicalAreas: data.clinicalAreas.filter(a => a !== area) 
      });
    } else {
      updateData({ 
        clinicalAreas: [...data.clinicalAreas, area] 
      });
    }
  };
    ...
    >
      <div className="space-y-6">
          ...
          <div className="flex space-x-4">
            <label className="flex items-center">
              <input
                ...
                onChange={() => updateData({ currentlyWorking: true })}
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={() => updateData({ currentlyWorking: false })}
                className="mr-2 text-blue-600"
              />
        ...
        <div>
          ...
          {data.clinicalAreas.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {data.clinicalAreas.map((area, index) => (
                <span
                  key={index}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {clinicalAreasOptions
              .filter(area => !data.clinicalAreas.includes(area))
              .map((area, index) => (
                <button
                  key={index}
                  onClick={() => toggleClinicalArea(area)}
